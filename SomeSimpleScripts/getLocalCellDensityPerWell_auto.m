...
if nargin==0
    ...
    %strRootPath = '/Users/dorispopovic/shares/pelkmans-new-fileserver/dpopovic/2019/17092019Doris4i-Cycle1/BATCH';
    strRootPath = '/Users/dorispopovic/shares/pelkmans-new-fileserver/dpopovic/2019/4i-Rab/01112019Doris4i-Rab-Cycle11/BATCH';

    %strRootPath = '/Volumes/Doris_5TB/2019/160319-FISHendosTreat03/BATCH';
...
cellNucleiMetaData = cell(size(handles.Measurements.Nuclei.Location));
% add offsets to nuclei positions
for i = 1:length(handles.Measurements.Nuclei.Location)
    if ~isempty(handles.Measurements.Nuclei.Location{i})
        % meta data: image-index & object-index
        cellNucleiMetaData{i} = NaN(size(handles.Measurements.Nuclei.Location{i}));
        cellNucleiMetaData{i}(:,1) = i;
        cellNucleiMetaData{i}(:,2) = 1:size(handles.Measurements.Nuclei.Location{i},1);
    end
end
...
for iOverlap = matOverlapValues
    ...
    cellNucleiPositions = handles.Measurements.Nuclei.Location;
    % add offsets to nuclei positions
...
elseif ~isempty(strfind(strRootPath,'101013_10-62_siRNAsize'))
    fprintf('%s: optimal overlap value of %d overwritten by FRANK setting of 100!\n',mfilename,intImageBorderOverlap)
